...
export const useMapTypeConfig = { BAIDU_OFFLINE: 'offlineMap', BAIDU: 'onlineMapGL', GOOGLE: 'googleMap', YANDEX: 'yandexMap' }
...
export class MapManager {
  ...
  static loaders = {
    ...
    async googleMap(type, ak = 'AIzaSyBMjaJ7_fQU-p2KIho28em8YpyilEeUSUM') {
  ...
  /**
   *
   * @param {('offlineMap' | 'onlineMap' | 'onlineMapGL'| 'googleMap' |'yandexMap')} type
   * @param {string} ak
   */
  ...
  static getAk(type) {
    const akMap = {
      offlineMap: '',
      onlineMap: store.state.baiduKey,
      onlineMapGL: store.state.baiduKey,
      googleMap: store.state.googleKey,
      yandexMap: store.state.yandexKey,
    }
    return akMap[type]
  }
}
